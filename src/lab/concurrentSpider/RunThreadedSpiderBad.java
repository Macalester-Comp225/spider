 ...
 *
 */
public class RunThreadedSpiderBad {
	...
	private static SharedSpiderDataBad sharedData = new SharedSpiderDataBad();
...
//		spider.crawl(BEGINNING_URL);
	...
	public static Thread startThread(int threadNum) {
		Thread t;
		if (threadNum != 0) {
			t = new Thread(new ConcurrentSpiderBad(sharedData));
		} else {
			t = new Thread(new ConcurrentSpiderBad(sharedData, BEGINNING_URL));
		}
