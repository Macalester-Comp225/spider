 ...
 *
 */
public class ConcurrentSpiderBad implements Runnable {
	...
	private SharedSpiderDataBad sharedData;
	 ...
	 * @param data
	 */
	public ConcurrentSpiderBad(SharedSpiderDataBad data) {
		sharedData = data;
	}
	/**
	 * Create a new spider with access to the shared data by passing it a reference.
	 ...
	 * @param startURL
	 */
	public ConcurrentSpiderBad(SharedSpiderDataBad data, String startURL) {
		sharedData = data;
	...
	public void run() {
		...
		if (beginningURL != null) {  
			try {
				sharedData.getWork().add(beginningURL);
